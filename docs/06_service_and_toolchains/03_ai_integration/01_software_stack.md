...
在已有 CLI 的前提下，桌面客户端不是替代层，而是面向多模态交互和业务交付的增强层。

### 5.1 定位

- CLI 负责自动化与脚本化编排，桌面客户端负责可视化协同与人工决策。
- 桌面客户端可直接连接 Aether 能力面（经 AE MCP Gateway），作为统一入口访问查询、分析与执行能力。
- 桌面客户端仍通过 Gateway 调用工具，不直接绕过 Propose→Commit 与审计链。
- 对外演示、阶段验收、交付汇报优先使用桌面端，降低沟通成本。

### 5.2 多模态交互价值

- 在同一工作区并排呈现文本结论、热力图、快照、时间线，支持联动定位。
- 模型结论附带证据卡片（截图、摘要、事件片段），便于快速复核。
- 对大结果集继续采用 View Token 句柄，客户端按需拉取渲染，不把原始数据注入模型上下文。

### 5.3 业务扩展价值

- 提供运营看板：任务量、冲突率、平均处置时长、回滚率等核心指标。
- 提供报表中心：日报/周报自动汇总，支持导出 PDF/CSV 用于交付。
- 提供复盘能力：按任务回放感知、分析、执行链路，直接对齐审计记录。
- 预留可视化操作面板：支持区域框选、对象点选、轨迹拖拽等交互，并映射为受控工具调用。

### 5.4 最小接入原则

- 先做最小可用桌面壳（多会话 + 多模态证据联动 + 报表导出），不改底层协议。
- 权限模型、错误码语义、工具契约与 CLI 保持一致，避免出现双套规则。
- 将桌面端定义为同一能力面的另一入口，统一由 Gateway 进行治理。
