...
## 3. 主子卡片机制与 LOD 分区优化 (Partition & LOD)
系统在分配实体存储时引入了极为优雅的“主卡片（Shared Card）”与“子卡片（Part Card）”解耦设计：
- **主卡片（Shared）**：存储图形核心数据的共享内存块，附带由一层底层架构自动维护的引用计数。
- **子卡片（Part）**：存放轻量的物理空间分区坐标，持有对主卡的引用。

当调用 `pyramid_insert` 插入图形时，引擎利用动态注册的 `partition_filter` 回调函数，从最佳层级 $k$ 开始向下逐层试探拆解。这种机制带来了极度霸道的**“插入即裁剪”**能力。例如，当挂载封装后封装的 `libclipper` 多边形运算库时：
开发者能够将庞大的复杂主结构（如角色的躯干大块）留在低精度网格层，而利用高精度手术刀（`clipper2`）切解出的细微附属结构（如手指）将直接作为**“子卡片”**沉降归档到更高精度层。
在此流程中，复杂的动态几何编辑（相交、挖空）被完美融合进空间索引的构建，且整个向下落盘过程**零数据拷贝 (Zero Data Copy)**。最终，外接的渲染器与物理引网仅需按当前视距扫描特定层级，无需拷贝和转移数据即可获得极低成本的 LOD (细节层次) 并行表现。
