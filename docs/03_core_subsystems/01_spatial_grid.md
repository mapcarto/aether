...
当调用 `pyramid_insert` 插入图形时，引擎利用动态注册的 `partition_filter` 回调函数，从最佳层级 $k$ 开始向下逐层试探拆解。这种机制允许开发者将庞大的复杂主结构（如角色的躯干大块）留在低精度网格层，而将细微的附属结构（如手指）沉降到更高精度层。渲染器与物理引擎仅需按当前视距扫描特定层级，无需拷贝和转移数据即可获得零成本的 LOD (细节层次) 表现。
