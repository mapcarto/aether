...
## 2. 核心调度：看门人单线程中枢 (Pyraman) 与快照机制
传统引擎多线程频繁读写树状金字塔极易引发死锁与数据污染，Aether 为此独创了名为 **Pyraman (看门人)** 的架构设计：
- **唯一的写特权收拢**：系统将并发瓶颈极限收拢，世界上“唯一拥有金字塔修改权限”的仅有 Pyraman 所在的单核心线程。外部所有的并发写意图均转化为事件流，投入 MPMC 交收给看门人入库序列化。
...
### 内存管理库选型性能对照
面对追求极致确定性的临时级内存突发频次（特别是单帧数百万量级绘制矩阵群瞬时派生与注销），`memarena` 精简了泛用型开源管理器为支持复杂跨线程生命周期所作的深层架构，换取了绝对意义上低开销的小组件吞吐下限：

| 评估指标 (Release) | memarena (深度客制化) | jemalloc | tcmalloc | mimalloc |
| --- | --- | --- | --- | --- |
