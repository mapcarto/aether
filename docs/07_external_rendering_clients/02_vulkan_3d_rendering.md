...
必须强调，外部 Vulkan 的帧率起伏不会引发 AE 框架“心脏骤停”。这套体系内，若是 Vulkan 因为某种重度着色器导致的渲染卡在 20 帧，AE 底层的事件循环依然以 8 亿次吞吐的物理基准坚定跑在自己所在的 CPU 独立线程集上，渲染端只管按照自己的步调，贪婪地索取它能拿到的“最新鲜”系统快照画面。
