...
1. **[空间计算底座 (Spatial Computing Foundation)](./docs/03_core_subsystems/index.md)**
   彻底摒弃传统引擎的黑盒内存分配与锁争用泥潭。依托 `memarena` 物理连贯内存、Pyraman 单核无锁调度与金字塔格网，Aether 在基准条件下单线程吞吐可达 800 万次/秒量级的纯 C 语言硬核下盘。
...
在基准条件下，单线程吞吐可达 **800 万次事件/秒** 量级；实际表现与硬件平台、工作集规模、编译参数相关（详见 `docs/05_benchmarks/01_performance_metrics.md`）。

---

## 与业界方案的定位区别 (Market Positioning)

为了清晰理解 Aether 的差异化位置，下表对标了空间计算领域的三类主流系统：

| 系统类型 | 代表产品 | 核心能力 | 应用特点 | Aether 优势 |
| :--- | :--- | :--- | :--- | :--- |
| **空间数据引擎** | ESRI SDE、PostGIS、OGC | 存储、索引、查询 | 离线/准实时查询、属性分析 | × 不是数据库 |
| **可视化服务** | Cesium、3DTiles、ArcGIS Server | 网络分发、渲染管线 | 前端展示优化、地图交互 | × 不是渲染引擎 |
| **分布式实时空间网格服务** | **Aether Server** | **实时计算、事件驱动、多actor协同** | **高频推演、AI决策反馈、并发控制** | ✅ 百万级实体、毫秒级延迟、无锁架构 |

Aether 专注于"实时推演与控制"的中间层——介于数据存储与前端展示之间，为 LLM agents、工业仿真、无人系统等需要**高频决策反馈**的场景提供算力底座。
