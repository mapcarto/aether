...
*   **部署建议与约束**：系统高度依赖底层硬件资源调度机制。为降低缺页中断、保障大页缓存效率并更精细控制 `SCHED_FIFO` 调度优先级与 `NUMA` 节点绑定，在低时延目标场景下建议优先采用 **Bare-metal (裸金属) 原生部署**。容器化方案（K8s / Docker）并非绝对不可用，但需经专项压测验证后再用于生产关键链路。
...
### 5.1 Aether Server 分布式实时协同集群

作为封装内核并向外提供算力的分布式框架，Aether Server 基于 C 语言与协程模型实现了严格的并发 I/O 隔离和高频跨进程通信调度：
...
*   **空间网格寻址吞吐 (Grid Access Throughput)**：在基准测试条件下（硬件、工作集、编译参数满足测试约束），凭借 O(1) 索引结构与 memarena 无锁通道的协同优化，系统吞吐可达到 **800 万次/秒以上** 的事件寻址与状态更新能力。
